...
for(let i=0; i<numSteps; i = i+1) {
  ...
  if (i == 0) {
    ...
    // Convert percentage to Roman numerals
    let percentValue = i * 10;
    // Only display Roman numerals 1-9
    if (percentValue >= 10 && percentValue <= 90) {
      let simpleNumber = Math.ceil(percentValue / 10);
      let romanPercent = toRoman(simpleNumber);
      text(romanPercent, 50, 280);
    }
  }
  translate(x_step, 0);
}
